...
class PlanningPokerAdapter {
    ...
    send(data) {
        ...
        try {
            ...
            if (!this.clientId) {
                console.error('clientId não definido. Envie uma mensagem "join" com username primeiro.')
                return
            }
    ...
    _checkAutoReveal() {
        ...
        const allDevelopersVoted = developers.every(dev => {
            return dev.selection !== null && dev.selection !== undefined && dev.selection !== ''
        })
...
class SupabaseWebSocket {
    ...
    set onopen(fn) { this._adapter.onopen = fn }
    set onmessage(fn) { this._adapter.onmessage = fn }
    set onclose(fn) { this._adapter.onclose = fn }
    set onerror(fn) { this._adapter.onerror = fn }
    ...
    destroy() {
        ...
        if (this._adapter._beforeUnloadHandler) {
            window.removeEventListener('beforeunload', this._adapter._beforeUnloadHandler)
            window.removeEventListener('pagehide', this._adapter._beforeUnloadHandler)
        }
        if (this._adapter._visibilityHandler) {
            document.removeEventListener('visibilitychange', this._adapter._visibilityHandler)
        }
        // Limpa heartbeat
